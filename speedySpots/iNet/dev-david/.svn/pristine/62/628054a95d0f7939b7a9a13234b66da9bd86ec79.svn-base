...
<% }
   else if (cancelHasHappened = true && IARequest.IACustomerCreditCardID > 0)
   { %>
   <div id="paymentPreAuthorizedandCancel" runat="server" class="message">
      <p>You have pre-authorized payment of this request, but since you have chosen to cancel your card WON'T be charged.</p>
   </div>
   <% } %>

    <% else if (cancelHasHappened = false && IARequest.IACustomerCreditCardID > 0)
    {%>
